...
export class ValidateProvider {
    ...
    public static FormatMessage<TValue>(methodRegister: IMethodRegister, value: TValue, params: any[], message?: string): string {
        ...
        if (format) {
            ...
            // 使用内置格式化
            return msg
                .replace(/{(\d)}/g, function(match: string, index: string) {
    ...
    public static PromisefyValidMethod<TValue>(methodRegister: IMethodRegister) {
        ...
        return function(value: TValue, ...params: any[]) {
            ...
            return new Promise<IMethodRegister>((resolve, reject) => {
                // 验证失败异常
                let validError = new ValidateMethodError(value, params, ValidateProvider.FormatMessage<TValue>(methodRegister, value, params, methodRegister.errMsg), name);
